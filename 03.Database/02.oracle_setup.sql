...
select password,username,user_id from dba_users;
...
select username, account_status,lock_date,password from dba_users where username = 'HR';

--사용자가 가지고 있는 테이블 조회하기
select * from tabs;


/*사용자 생성하기
  
	실습.사용자 확인, HR과 SCOTT 각각 확인해보기
	키워드는 대소문자를 구분하지만 값(문자열,'')은 대소문자를 구분을 한다
...
--1.사용자 계정 생성하기 // hr은 사용자 계정 생성 불가
--계정과 비밀번호를 동시에 적용 명령
...
--role 확인하기
select*from dba_role_privs
 where grantee = 'HR' OR grantee = 'SCOTT';
--권한 확인하기
select * from dba_sys_privs 
where grantee = 'HR' OR grantee = 'SCOTT';
...
grant connect to scott; -- connect (db 접속) 권한 부여
