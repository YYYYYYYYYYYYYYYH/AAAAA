...
--1.레코드 추가 
--실습1. dept2에 부서번호=9000, 부서명=테스트부서_1, 상위부서=1006, 지역 = 기타지역 으로 추가해보기 
...
insert into dept2 (dcode,dname,pdept,area) values (9001,'테스트부서_3',1006,'기타지역');  -- 에러
...
insert into professor (profno,name,id,position,pay,hiredate) values (5001,'홍길동','hong','정교수',510,'20230327');

--실습3. 
--1) professor 의 구조만 복사해서 professor4 
--2) professor 에서 profno가 4000보다 큰 교수만 professor4 추가
...
--실습4.
--professor을 기준으로 pro_3과 pro_4 테이블을 생성
...
--update 주의할점 
update emp999
 set ename = '스미스'; -- ename 전체가 스미스로 바뀜
...
--실습3. professor1 에서 직급이 assistant professor 인 사람의 보너스를 200으로 인상하기

update professor1
 set bonus  = 200
where position = 'assistant professor'	;
 
--실습 4. professor1 에서 Sharon Stone과 직급이 돌일한 교수들의 급여를 15% 인상하기
--hint) 서브쿼리를 이용 where 절에 서브쿼리를 지정 where position = (select from professor where name 샤론스톤의 포지션)
...
update professor1
 set pay  = pay * 1.15
where position =  (select* from professor1 where name = 'Sharon Stone')	; -- too many values
  ...
  merge into 병합할테이블명 
			  using 테이블1 on 병합할 조건 
				when matched then update set 업데이트할 명령 
				              delete where 조건절 when not matched then insert into value(....);
*/
...
E. transaction
 ...
 에러가 없을 경우에는 최종적으로 작업을 확정하게 되는데 이런 작업을 취소 or 확정하게 하는 명령어 rollback, commit 이다
