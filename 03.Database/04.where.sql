 ...
 <=
  ...
  2. 기타연산자
	...
	A or B : 논리합
	not A : 부정 
	between A and B : a와b 사이의 데이터를 검색, a는 b보다 작은값으로 정의 
...
select * from emp where sal <> 900;  -- 900이 아닌사람 
select * from emp where sal != 900;  -- 900이 아닌사람 

--3. 이름이 smith 인 사원 조회하기
select * from emp where ename = 'SMITH';
select * from emp where ename = SMITH ; -- 에러 / SMITH 을 열이름으로 인식 
...
-- 4. 입사일자(hiredate)
-- 입사일자가 1980-12-17인 사원을 조회 (hint : data 타입은 비교할 때 문자열로 간주)
