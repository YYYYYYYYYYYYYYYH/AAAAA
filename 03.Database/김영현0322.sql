...
select empno,ename, hiredate from emp where to_char(hiredate,'MM')='01' or to_char(hiredate,'MM')='02' or to_char(hiredate,'MM')='03' ;
...
select name 이름 ,pay 급여,nvl(bonus,0) 보너스 , to_char((pay*12)+ nvl(bonus,0),'99,999')연봉 from professor where deptno = 201 ; 
...
select * from emp;
select empno,ename,to_char(hiredate,'YYYY-MM-DD') hiredate, to_char((sal*12)+comm,'$99,999') 총연봉 ,to_char(((sal*12)+comm)*1.15,'$99,999') "15%인상한총연봉" from emp where comm is not null;
